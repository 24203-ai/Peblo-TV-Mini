...
def get_seasons(show_id: str = None, db: Session = Depends(get_db), current_user = Depends(get_current_user), skip: int = 0, limit: int = 100):
...
def get_season(season_id: str, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def update_season(season_id: str, season_in: SeasonUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def delete_season(season_id: str, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    db_season = db.query(Season).filter(Season.id == season_id).first()
    if not db_season:
        raise HTTPException(status_code=404, detail="Season not found")
    db.delete(db_season)
